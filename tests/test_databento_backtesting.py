...
class TestDataBentoDataBacktesting(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_asset = Asset(symbol="ES", asset_type="future", expiration=datetime(2025, 3, 15).date())

    @patch("lumibot.tools.databento_helper.DATABENTO_AVAILABLE", True)
    def test_initialization_success(self):
        """Test successful initialization"""
        backtester = DataBentoDataBacktesting(
            datetime_start=self.start_date, datetime_end=self.end_date, api_key=self.api_key
        )
    ...
    def test_initialization_databento_unavailable(self):
        ...
        with self.assertRaises(ImportError):
            DataBentoDataBacktesting(datetime_start=self.start_date, datetime_end=self.end_date, api_key=self.api_key)
    ...
    def test_update_pandas_data_success(self, mock_get_data):
        ...
        test_df.index = pd.to_datetime(["2025-01-01 09:30:00", "2025-01-01 09:31:00", "2025-01-01 09:32:00"])
        ...
        with patch.object(backtester, "get_start_datetime_and_ts_unit") as mock_get_start:
            ...
            # Call update method
            backtester._update_pandas_data(asset=self.test_asset, quote=None, length=10, timestep="minute")
    ...
    def test_update_pandas_data_existing_sufficient_data(self, mock_get_data):
        ...
        search_asset = (self.test_asset, Asset("USD", "forex"))
        existing_data = Data(self.test_asset, df=existing_df, timestep="minute", quote=Asset("USD", "forex"))
    ...
    def test_get_last_price_from_cached_data(self):
        ...
        search_asset = (self.test_asset, Asset("USD", "forex"))
        data = Data(self.test_asset, df=test_df, timestep="minute", quote=Asset("USD", "forex"))
    ...
    def test_get_bars_dict_success(self):
        ...
        with patch.object(backtester, "_update_pandas_data"):
            result = backtester._get_bars_dict(assets=[self.test_asset], length=2, timestep="minute")
    ...
    def test_get_bars_dict_with_timeshift(self):
        ...
        test_df.index = pd.to_datetime(
            ["2025-01-01 09:30:00", "2025-01-01 09:31:00", "2025-01-01 09:32:00", "2025-01-01 09:33:00"]
        )
        ...
        with patch.object(backtester, "_update_pandas_data"):
            ...
            result = backtester._get_bars_dict(
                assets=[self.test_asset], length=2, timestep="minute", timeshift=timedelta(minutes=1)
            )
    ...
    def test_prefetch_data_multiple_assets(self, mock_get_data):
        ...
        test_assets = [Asset("ESH23", "future"), Asset("NQH23", "future")]
