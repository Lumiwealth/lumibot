"""Polygon backtesting data source implemented with polars.

This implementation:
1. Eliminates datalines - uses polars columnar storage directly
2. Avoids pandas conversions
...
class PolygonDataBacktestingPolars(PolarsData):
    ...
    def _store_data(self, asset: Asset, data: pl.DataFrame) -> pl.LazyFrame:
        ...
        """
        # Ensure dividend column exists so downstream calculations have a stable schema
        if "dividend" not in data.columns:
            data = data.with_columns(pl.lit(0.0).cast(pl.Float64).alias("dividend"))
        else:
            data = data.with_columns(pl.col("dividend").cast(pl.Float64))
    ...
    def _update_data(self, asset: Asset, quote: Asset, length: int, timestep: str, start_dt=None):
        ...
        # Clamp the requested range so we never ask Polygon for data beyond what should exist
        effective_end = self.datetime_end
        if isinstance(effective_end, datetime):
            real_now_utc = datetime.now(timezone.utc)
            if effective_end.tzinfo is not None:
                real_now = real_now_utc.astimezone(effective_end.tzinfo)
            else:
                real_now = real_now_utc.replace(tzinfo=None)
            if real_now < effective_end:
                effective_end = real_now
        ...
        try:
            ...
            formatted_end_datetime = effective_end.strftime("%Y-%m-%d")
            text = str(e)
            plan_msgs = (
    ...
    def _pull_source_symbol_bars(
        ...
        # Build search key consistent with storage logic
        quote_asset = quote if quote is not None else Asset("USD", "forex")
        if isinstance(asset, tuple):
            search_asset = asset
        else:
    ...
    def get_last_price(
        ...
        **kwargs
        ...
        timeshift = None if timestep == "day" else None
        length = 1
    ...
    def get_historical_prices(
        ...
        if isinstance(asset, str):
            ...
            if ticker.endswith("=X") or ticker.startswith("/"):
                asset = Asset(asset, asset_type=Asset.AssetType.FOREX)
            elif ticker.endswith(".X"):
                asset = Asset(asset, asset_type=Asset.AssetType.CRYPTO)
            else:
    ...
    def get_historical_prices_between_dates(
        ...
        if start_date or end_date:
            ...
            start_dt_norm = _normalize(start_date)
            end_dt_norm = _normalize(end_date)

            filters = []
            if start_dt_norm:
                # Use pl.lit() to ensure datetime precision compatibility across Polars versions
                filters.append(pl.col("datetime") >= pl.lit(start_dt_norm))
            if end_dt_norm:
                # Use pl.lit() to ensure datetime precision compatibility across Polars versions
                filters.append(pl.col("datetime") <= pl.lit(end_dt_norm))
