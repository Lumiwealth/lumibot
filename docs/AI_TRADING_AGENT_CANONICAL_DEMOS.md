...
**Tool:** `get_fred_series` -- wraps `https://fred.stlouisfed.org/graph/fredgraph.csv` for a simple public macro-data demo. This endpoint returns revised data, so use the built-in FRED tools with `FRED_API_KEY` for strict point-in-time macro backtests.
