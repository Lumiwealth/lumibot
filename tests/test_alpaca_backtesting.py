...
from tests.fixtures import BacktestingTestStrategy, BaseDataSourceTester
...
class TestAlpacaBacktesting:
    ...
    def _create_data_source(
        self,
        *,
        datetime_start=datetime(2025, 1, 1, tzinfo=NY_TZ),
        datetime_end=datetime(2025, 1, 31, tzinfo=NY_TZ),
    ...
    def test_basic_key_generation_stock(self):
        datetime_start = datetime(2025, 1, 1, tzinfo=NY_TZ)
        datetime_end = datetime(2025, 2, 1, tzinfo=NY_TZ)
    ...
    def test_with_auto_adjust(self):
        ...
        market = "NYSE"
        timestep = "day"
        start_date = datetime(2023, 1, 1, tzinfo=NY_TZ)
        end_date = datetime(2023, 12, 31, tzinfo=NY_TZ)
        auto_adjust = True

        data_source = self._create_data_source(
            datetime_start=start_date, datetime_end=end_date, market=market, timestep=timestep, auto_adjust=auto_adjust
        )
    ...
    def test_empty_market_false_auto_adjust(self):
        ...
        data_source = self._create_data_source(
            datetime_start=start_date, datetime_end=end_date, timestep=timestep, auto_adjust=auto_adjust
        )
    ...
    def test_different_timezones(self):
        ...
        market = "24/7"
        timezones = [pytz.timezone("Asia/Tokyo"), pytz.timezone("Europe/London"), pytz.timezone("US/Pacific")]
        timestep = "day"
        ...
        data_source = self._create_data_source(
            datetime_start=start_date, datetime_end=end_date, market=market, timestep=timestep
        )
        ...
        for tz, expected in zip(timezones, expected_results, strict=False):
            key = data_source._get_asset_key(
                base_asset=base_asset,
                quote_asset=quote_asset,
                market=market,
    ...
    def test_refresh_cache(self):
        ...
        key = data_source._get_asset_key(base_asset=base_asset, quote_asset=quote_asset)
    ...
    def test_reindex_and_fill_day_when_all_data_exists(self):
        datetime_start = datetime(2025, 1, 13, tzinfo=NY_TZ)
        datetime_end = datetime(2025, 1, 17, tzinfo=NY_TZ)
        market = "NYSE"
        timestep = "day"
    ...
    def test_amzn_day_1d(
        self,
        asset: Asset = AMZN_ASSET,
        market: str = "NYSE",
        timestep: str = "day",
        sleeptime: str = "1D",
        tzinfo: pytz.tzinfo = NY_TZ,
        auto_adjust: bool = True,
        warm_up_trading_days: int = 0,
    ...
    def test_amzn_day_1d_dump_benchmark_stats(
        ...
        results, strategy = BuyAndHold.run_backtest(
            ...
            backtesting_end=backtesting_end,
            minutes_before_closing=0,
            benchmark_asset=AMZN_ASSET,
            analyze_backtest=False,
            show_progress_bar=False,
            parameters={"buy_symbol": "AMZN"},
            # AlpacaBacktesting kwargs
            timestep=timestep,
            market=market,
            config=ALPACA_TEST_CONFIG,
        ...
        expected_start = tzinfo.localize(datetime(2025, 1, 13, 0, 0))
        assert strategy._benchmark_returns_df.index[0] == expected_start
    ...
    def test_amzn_day_1d_benchmark_asset_loaded_when_benchmark_asset_is_crypto(
        ...
        expected_start = tzinfo.localize(datetime(2025, 1, 13, 0, 0))
        assert strategy._benchmark_returns_df.index[0] == expected_start
        assert strategy._benchmark_returns_df.iloc[0].open == 94558.75
    ...
    def test_oauth_config_backtesting(self):
        ...
        oauth_config = {"OAUTH_TOKEN": "test_oauth_backtesting_token", "PAPER": True}

        datetime_start = datetime(2025, 1, 1, tzinfo=NY_TZ)
        datetime_end = datetime(2025, 1, 31, tzinfo=NY_TZ)

        data_source = AlpacaBacktesting(
            datetime_start=datetime_start, datetime_end=datetime_end, config=oauth_config, timestep="day"
        )
    ...
    def test_oauth_mixed_config_backtesting(self):
        ...
        data_source = AlpacaBacktesting(
            datetime_start=datetime_start, datetime_end=datetime_end, config=mixed_config, timestep="day"
        )
    ...
    def test_oauth_fallback_backtesting(self):
        ...
        data_source = AlpacaBacktesting(
            datetime_start=datetime_start, datetime_end=datetime_end, config=fallback_config, timestep="day"
        )
...
class TestAlpacaBacktestingDataSource(BaseDataSourceTester):
    ...
    def test_get_last_price_daily_bars_crypto(self):
        ...
        data_source._datetime = now
        price = data_source.get_last_price(asset=asset)
        assert price == 98411.446  # open price of the daily bar
        assert isinstance(price, float)
    ...
    def test_get_last_price_minute_bars_crypto(self):
        ...
        data_source._datetime = now
        price = data_source.get_last_price(asset=asset)
        assert price == 96568.9324757  # open price of the minute bar
        assert isinstance(price, float)

        now = tzinfo.localize(datetime(2025, 2, 21, 9, 30))
        data_source._datetime = now
        price = data_source.get_last_price(asset=asset)
        assert price == 98416.825  # open price of the minute bar

        now = tzinfo.localize(datetime(2025, 2, 21, 15, 59))
        data_source._datetime = now
        price = data_source.get_last_price(asset=asset)
        assert price == 98032.45  # open price of the minute bar
    ...
    def test_get_historical_prices_minute_bars_stock(self):
        ...
        for length in [1, 30]:
            bars = data_source.get_historical_prices(
                asset=asset, length=length, timestep=timestep, include_after_hours=True
            )
            ...
            self.check_minute_bars(bars=bars, now=now, data_source_tz=data_source.tzinfo, market=market)
    ...
    def test_get_historical_prices_minute_bars_stock_remove_incomplete_current_bar(self):
        ...
        for length in [1, 30]:
            ...
            self.check_index(bars=bars, data_source_tz=data_source.tzinfo)
            self.check_minute_bars(
                bars=bars, now=now, data_source_tz=data_source.tzinfo, market=market, remove_incomplete_current_bar=True
            )
    ...
    def test_get_historical_prices_daily_bars_stock(self):
        ...
        for length in [1, 30]:
            ...
            self.check_daily_bars(
                bars=bars, now=now, data_source_tz=data_source.tzinfo, time_check=time(0, 0), market=market
            )
    ...
    def test_get_historical_prices_daily_bars_crypto(self):
        tzinfo = pytz.timezone("UTC")
        datetime_start = datetime(2025, 1, 1, tzinfo=pytz.timezone("UTC"))
        datetime_end = datetime(2025, 3, 1, tzinfo=pytz.timezone("UTC"))
        market = "24/7"
        timestep = "day"
        asset = Asset("BTC", "crypto")
