...
class TestTimezoneHandling:
    ...
    def test_datetime_timezone_consistency_in_databento_context(self):
        ...
        df = pd.DataFrame({"timestamp": [start_date, end_date], "value": [1, 2]})
        df.set_index("timestamp", inplace=True)
...
class TestDataBentoIntegration:
    ...
    def test_databento_futures_asset_validation(self):
        ...
        result = data_source.get_historical_prices(asset=stock_asset, length=10, timestep="minute")
        ...
        try:
            ...
            data_source.get_historical_prices(asset=futures_asset, length=10, timestep="minute")
...
class TestPolarsDSTHandling:
    ...
    def test_clean_trading_times_handles_dst_fallback(self):
        ...
        expected_minutes = (
            int((market_close.astimezone(pytz.UTC) - market_open.astimezone(pytz.UTC)).total_seconds() / 60) + 1
        )
    ...
    def test_databento_supported_asset_types(self):
        ...
        try:
            data_source.get_historical_prices(asset=cont_future_asset, length=5, timestep="minute")
            assert True
        except ValueError as e:
            if "only supports futures assets" in str(e):
...
class TestDataBentoTimezoneFixValidation:
    ...
    def test_dataframe_timezone_operations(self):
        ...
        df = pd.DataFrame({"timestamp": timestamps, "close": [100 + i for i in range(10)]})
        df.set_index("timestamp", inplace=True)
