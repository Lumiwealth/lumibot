...
class TestBrokerCleanup(unittest.TestCase):
    ...
    def test_custom_cleanup_config_initialization(self):
        ...
        custom_config = {
            ...
            "retention_policies": {"filled_orders": {"max_age_days": 15, "max_count": 5000}},
        }

        # Create a fresh broker for this test to avoid interference from other tests
        fresh_mock_data_source = MockDataSource()
        broker = MockBroker(
            name="test_broker", connect_stream=False, data_source=fresh_mock_data_source, cleanup_config=custom_config
        )
    ...
    def test_cleanup_respects_min_keep(self):
        ...
        removed_count = self.broker._cleanup_tracking_list(self.broker._filled_orders, policy, current_time)
    ...
    def test_cleanup_count_based_retention(self):
        ...
        expected_dates = [current_time - timedelta(days=i) for i in range(3)]
