...
from lumibot.tools.lumibot_logger import BotspotErrorHandler, BotspotSeverity, get_logger, get_strategy_logger
...
class TestBotspotErrorHandler:
    ...
    def test_handler_initialization_with_api_key(self):
        ...
        with patch.dict(os.environ, {"LUMIWEALTH_API_KEY": "test-api-key"}):
            with patch("lumibot.tools.lumibot_logger.LUMIWEALTH_API_KEY", None):
                handler = BotspotErrorHandler()
    ...
    def test_extract_error_info_with_strategy(self):
        ...
        record = logging.LogRecord(
            ...
            pathname="test.py",
            lineno=10,
            msg="[MyStrategy] Strategy error occurred",
            args=(),
            exc_info=None,
            func="test_func",
        )
    ...
    def test_extract_error_info_structured_format(self):
        ...
        record = logging.LogRecord(
            ...
            pathname="test.py",
            lineno=10,
            msg="CUSTOM_ERROR: Something went wrong | Additional context here",
            args=(),
            exc_info=None,
            func="test_func",
        )
    ...
    def test_extract_error_info_strategy_with_structured_format(self):
        ...
        record = logging.LogRecord(
            ...
            pathname="test.py",
            lineno=10,
            msg="[MyStrategy] CUSTOM_ERROR: Something went wrong | Additional context",
            args=(),
            exc_info=None,
            func="test_func",
        )
    ...
    def test_extract_error_info_strategy_name_in_message(self):
        ...
        record = logging.LogRecord(
            ...
            pathname="test.py",
            lineno=10,
            msg="[MyStrategy] Failed to process MyStrategy configuration",
            args=(),
            exc_info=None,
            func="test_func",
        )
    ...
    def test_extract_error_info_no_double_spacing(self):
        ...
        record = logging.LogRecord(
            ...
            pathname="test.py",
            lineno=10,
            msg="[MyStrategy]  Double space after prefix",
            args=(),
            exc_info=None,
            func="test_func",
        )
    ...
    def test_report_to_botspot_success(self, mock_post):
        ...
        with patch.dict(os.environ, {"LUMIWEALTH_API_KEY": "test-api-key"}):
            ...
            result = handler._report_to_botspot(
                BotspotSeverity.CRITICAL, "TEST_ERROR", "Test message", "Test details", 1
            )
    ...
    def test_duplicate_aggregation(self):
        ...
        with patch.dict(os.environ, {"LUMIWEALTH_API_KEY": "test-api-key", "BOTSPOT_RATE_LIMIT_WINDOW": "1"}):
            with patch("lumibot.tools.lumibot_logger.LUMIWEALTH_API_KEY", None):
                handler = BotspotErrorHandler()
    ...
    def test_rate_limit_window(self):
        ...
        with patch.dict(os.environ, {"LUMIWEALTH_API_KEY": "test-api-key", "BOTSPOT_RATE_LIMIT_WINDOW": "2"}):
            with patch("lumibot.tools.lumibot_logger.LUMIWEALTH_API_KEY", None):
                handler = BotspotErrorHandler()
                handler._report_to_botspot = MagicMock(return_value=True)
    ...
    def test_rate_limit_counter_reset(self):
        ...
        with patch.dict(os.environ, {"LUMIWEALTH_API_KEY": "test-api-key", "BOTSPOT_MAX_ERRORS_PER_MINUTE": "2"}):
            with patch("lumibot.tools.lumibot_logger.LUMIWEALTH_API_KEY", None):
                handler = BotspotErrorHandler()
                handler._report_to_botspot = MagicMock(return_value=True)
...
class TestBotspotIntegration:
    ...
    def test_handler_not_added_without_api_key(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            with patch("lumibot.tools.lumibot_logger.LUMIWEALTH_API_KEY", None):
                ...
                root_logger = logging.getLogger("lumibot")
                botspot_handlers = [h for h in root_logger.handlers if isinstance(h, BotspotErrorHandler)]
    ...
    def test_strategy_logger_integration(self):
        ...
        record = logging.LogRecord(
            ...
            pathname="test.py",
            lineno=10,
            msg="[TestStrategy] Strategy execution failed",
            args=(),
            exc_info=None,
            func="test_func",
        )
