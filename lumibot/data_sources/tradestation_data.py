...
class TradeStation(DataSource):
    ...
    def __init__(self, config, max_workers=20, chunk_size=100, **kwargs):
        ...
        # it's best practice to initialize Tradestation with the url defined in the config. Otherwise, for safety reasons, we default to the sandbox.
        if hasattr(config, "ENDPOINT"):
            self.endpoint = URL(config.ENDPOINT)
        else:
            self.endpoint = URL("https://sim-api.tradestation.com/v3")
        if hasattr(config, "VERSION"):
            self.version = config.VERSION
        else:
            self.version = "v3" #using v3 by default
