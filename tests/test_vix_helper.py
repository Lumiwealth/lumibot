...
class TestVixHelper(unittest.TestCase):
    ...
    def test_get_vix_value(self, mock_ticker_class):
        ...
        mock_df = pd.DataFrame(
            {"Open": [19.5, 20.0, 18.8, 19.2, 20.5], "Close": [20.5, 21.0, 19.8, 20.2, 21.5]},
    ...
    def test_get_vix_rsi_value(self, mock_ticker_class):
        ...
        mock_df = pd.DataFrame({"Open": [p - 0.5 for p in prices], "Close": prices}, index=dates)
        mock_ticker.history.return_value = mock_df
    ...
    def test_vix_percentile_calculation(self, mock_ticker_class):
        ...
        mock_df = pd.DataFrame({"Open": [v - 0.5 for v in vix_values[:100]], "Close": vix_values[:100]}, index=dates)
        mock_ticker.history.return_value = mock_df
