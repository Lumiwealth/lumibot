...
class TestBacktestingDataSources:
    ...
    def check_dividends_and_adjusted_returns(self, bars):
        ...
        comparison_df = pd.concat([actual_df["actual_return"], expected_df["expected_return"]], axis=1).reindex(
            actual_df.index
        )
    ...
    def test_pandas_backtesting_data_source_get_historical_prices_daily_bars_dividends_and_adj_returns(
        ...
        data_source = PandasData(
            datetime_start=datetime_start, datetime_end=datetime_end, pandas_data=pandas_data_fixture
        )
    ...
    def test_polygon_backtesting_data_source_get_historical_prices_daily_bars_for_backtesting_broker(self):
        ...
        data_source = PolygonDataBacktesting(datetime_start, datetime_end, api_key=POLYGON_CONFIG["API_KEY"])
        ...
        data_source._datetime = now
        bars = data_source.get_historical_prices(asset=asset, length=length, timeshift=timeshift, timestep=timestep)
        ...
        if bars is None or bars.df is None or bars.df.empty:
            pytest.skip(
                "Polygon API returned no data - possibly due to rate limits, invalid API key, or data availability"
            )
    ...
    def test_yahoo_backtesting_data_source_get_historical_prices_daily_bars_dividends_and_adj_returns(self):
        """
        ...
        data_source = YahooDataBacktesting(datetime_start=datetime_start, datetime_end=datetime_end)
        data_source._datetime = now
        bars = data_source.get_historical_prices(asset=asset, length=length, timestep=timestep)
        self.check_dividends_and_adjusted_returns(bars)

    @pytest.mark.skip(
        reason="CCXT Kraken integration test requires stable network connection and external API availability"
    )
    def test_kraken_ccxt_backtesting_data_source_get_historical_prices_daily_bars(self):
        """
        ...
        data_source = CcxtBacktesting(datetime_start=datetime_start, datetime_end=datetime_end, **kwargs)
        data_source._datetime = now
        bars = data_source.get_historical_prices(asset=(base, quote), length=length, timestep=timestep)
