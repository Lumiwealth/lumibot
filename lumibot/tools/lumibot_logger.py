...
# Unified logging intentionally accepts dynamic LogRecord payloads and third-party request clients.
# Keep that boundary explicit here instead of scattering casts through every handler path.
# pyright: reportArgumentType=false, reportConstantRedefinition=false, reportMissingParameterType=false
# pyright: reportMissingTypeArgument=false, reportUnknownArgumentType=false, reportUnknownMemberType=false
...
class BotspotErrorHandler(logging.Handler):
    ...
    def __init__(self):
        ...
        if self.api_key:
            try:
                ...
                logging.getLogger(__name__).warning("requests library not available - Botspot error reporting disabled")
                self.requests = None
        else:
            self.requests = None
    ...
    def _report_to_botspot(
        self, severity: BotspotSeverity, error_code: str, message: str, details: str, count: int
    ) -> bool:
        ...
        try:
            response = self.requests.post(self.base_url, headers=headers, json=payload, timeout=10)
    ...
    def emit(self, record):
        ...
        if not self.api_key or record.levelno < logging.ERROR:
            return
        reported_by = getattr(record, "_lumibot_botspot_reported_by", None)
        if reported_by is not None and reported_by != id(self):
            return
        record._lumibot_botspot_reported_by = id(self)
        try:
            with self._lock:
                now = time.time()
...
def set_log_level(level: str):
    ...
    try:
        ...
        raise ValueError(f"Invalid log level: {level}. Must be one of: DEBUG, INFO, WARNING, ERROR, CRITICAL") from None
