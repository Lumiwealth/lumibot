...
from lumibot.strategies.session_manager import BacktestingSession, LiveTradingSession, SessionManager
...
def timeout_handler(signum, frame):
    ...
    raise TestTimeoutError("Test timed out - possible infinite loop detected")
...
class TestSessionManager(unittest.TestCase):
        ...
        try:
            ...
            for i in range(max_iterations):
                ...
                if len(times_advanced) > 1:
                    self.assertGreater(
                        times_advanced[-1], times_advanced[-2], "Time must progress forward to prevent infinite loops"
                    )
    ...
    def test_backtesting_session_prevents_infinite_restart(self):
        ...
        try:
            ...
            self.assertLess(iterations, max_safe_iterations, "Session should complete within reasonable iterations")
            self.assertGreater(iterations, 0, "Session should execute at least one iteration")
    ...
    def test_live_trading_session_time_management(self):
        ...
        if start_time is not None:
            self.assertGreater(session._last_execution_time, start_time, "Live trading should update execution time")
    ...
    def test_session_architecture_separation_of_concerns(self):
        ...
        self.assertNotEqual(
            type(backtesting_session), type(live_session), "Sessions should be different classes for different concerns"
        )
    ...
    def test_data_source_agnostic_design(self):
        ...
        self.assertGreater(
            session._current_time, datetime(2024, 1, 1), "Time should progress independently of data source"
        )
