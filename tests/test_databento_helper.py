...
class TestDataBentoHelper(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_asset_future = Asset(symbol="ES", asset_type="CONT_FUTURE")
        self.test_asset_stock = Asset(symbol="AAPL", asset_type="stock")
    ...
    def test_format_futures_symbol_for_databento(self):
        ...
        specific_future = Asset(symbol="ES", asset_type="future", expiration=datetime(2025, 3, 15).date())
        ...
        dec_asset = Asset(symbol="ES", asset_type="future", expiration=datetime(2024, 12, 15).date())
    ...
    def test_databento_client_initialization(self):
        ...
        client = databento_helper.DataBentoClient(api_key=self.api_key, timeout=30, max_retries=3)
    ...
    def test_get_price_data_from_databento_success(self):
        ...
        es_asset = Asset(symbol="ES", asset_type=Asset.AssetType.CONT_FUTURE)

        result = databento_helper.get_price_data_from_databento(
            api_key=api_key, asset=es_asset, start=start_date, end=end_date, timestep="minute"
        )
    ...
    def test_get_price_data_databento_unavailable(self):
        ...
        result = databento_helper.get_price_data_from_databento(
            api_key=self.api_key, asset=self.test_asset_future, start=self.start_date, end=self.end_date
        )
    ...
    def test_build_cache_filename(self):
        ...
        filename = databento_helper._build_cache_filename(
            self.test_asset_future, self.start_date, self.end_date, "minute"
        )
    ...
    def test_no_retry_logic_for_correct_symbol(self):
        ...
        result = databento_helper.get_price_data_from_databento(
            api_key=api_key, asset=mes_asset, start=start_date, end=end_date, timestep="minute"
        )
    ...
    def test_futures_month_code_consistency(self):
        ...
        for expiration_date, expected_month_code in test_cases:
            with self.subTest(expiration=expiration_date):
                asset = Asset(symbol="ES", asset_type="future", expiration=expiration_date)
    ...
    def test_error_handling_empty_dataframe(self, mock_client_class):
        ...
        with patch("lumibot.tools.databento_helper._load_cache", return_value=None), patch(
            "lumibot.tools.databento_helper._save_cache"
        ):
            result = databento_helper.get_price_data_from_databento(
                api_key=self.api_key, asset=self.test_asset_stock, start=self.start_date, end=self.end_date
            )
    ...
    def test_cache_filename_edge_cases(self):
        ...
        for asset in assets:
            with self.subTest(asset=asset):
                filename = databento_helper._build_cache_filename(asset, self.start_date, self.end_date, "minute")
    ...
    def test_get_last_price_from_databento_success(self, mock_historical):
        ...
        # Call function
        result = databento_helper.get_last_price_from_databento(api_key="test_key", asset=asset)
    ...
    def test_timezone_aware_datetime_usage(self):
        ...
        with patch("lumibot.tools.databento_helper.Historical") as mock_historical:
            mock_df = pd.DataFrame({"close": [5025.0]})
            ...
            self.assertEqual(start_dt.tzinfo, UTC, "start datetime should use UTC timezone")
            self.assertEqual(end_dt.tzinfo, UTC, "end datetime should use UTC timezone")
    ...
    def test_get_price_data_continuous_futures_resolution(self, mock_client_class):
        ...
        # Call the function
        databento_helper.get_price_data_from_databento(
    ...
    def test_get_last_price_resolves_once_no_retry(self, mock_historical_class):
        ...
        with patch.object(es_cont, "resolve_continuous_futures_contract", return_value="ESH25"):
            # Call the function
            databento_helper.get_last_price_from_databento(api_key="test_api_key", asset=es_cont)
    ...
    def test_no_get_potential_futures_contracts_usage(self):
        ...
        self.assertNotIn(
            "get_potential_futures_contracts",
            source,
    ...
    def test_market_calendar_spanning_sessions(self):
        ...
        def mock_market_hours(close=True, next_day=False):
            if next_day:  # Friday's session
                if close:
                    return datetime(2025, 1, 10, 23, 0, tzinfo=UTC)  # 6pm ET Fri
                else:
                    return datetime(2025, 1, 9, 23, 0, tzinfo=UTC)  # 6pm ET Thu
            else:  # Thursday's session
                if close:
                    return datetime(2025, 1, 9, 23, 0, tzinfo=UTC)  # 6pm ET Thu
                else:
                    return datetime(2025, 1, 8, 23, 0, tzinfo=UTC)  # 6pm ET Wed

        def mock_utc_to_local(utc_time):
            # Convert UTC to ET (UTC-5) and return as naive datetime
            from dateutil import tz

            return utc_time.replace(tzinfo=UTC).astimezone(tz=tz.tzlocal()).replace(tzinfo=None)
        ...
        try:
            open_time_today = mock_utc_to_local(mock_market_hours(close=False, next_day=False))
            close_time_today = mock_utc_to_local(mock_market_hours(close=True, next_day=False))
        ...
        try:
            open_time_tomorrow = mock_utc_to_local(mock_market_hours(close=False, next_day=True))
            close_time_tomorrow = mock_utc_to_local(mock_market_hours(close=True, next_day=True))
...
class TestDataBentoAuthenticationRetry(unittest.TestCase):
    ...
    def test_authentication_retry_on_401_error(self, mock_historical):
        ...
        with patch("lumibot.tools.databento_helper.Historical", mock_historical):
            result = self.client.get_historical_data(
                dataset="GLBX.MDP3", symbols="MESU5", schema="ohlcv-1m", start="2025-01-01", end="2025-01-02"
            )
    ...
    def test_non_auth_errors_not_retried(self, mock_historical):
        ...
        with self.assertRaises(Exception) as context:
            self.client.get_historical_data(
                dataset="GLBX.MDP3", symbols="INVALID", schema="ohlcv-1m", start="2025-01-01", end="2025-01-02"
            )
                ...
                with patch("lumibot.tools.databento_helper.Historical", mock_historical):
                    try:
                        result = client.get_historical_data(
                            dataset="GLBX.MDP3", symbols="TEST", schema="ohlcv-1m", start="2025-01-01", end="2025-01-02"
                        )
                        # If auth-related error, should retry and succeed
                        if any(
                            keyword in error_msg.lower()
                            for keyword in ["401", "auth", "authentication", "unauthorized", "token"]
                        ):
                            self.assertIsInstance(result, pd.DataFrame)
