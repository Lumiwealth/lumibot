...
class TestDailyDataTimestampComparison:
    ...
    def _test_symbol_daily_data(self, symbol, start_date, end_date, min_trading_days, asset_type="stock"):
        ...
        try:
            theta_df = thetadata_helper.get_price_data(
                username=username, password=password, asset=asset, start=start_date, end=end_date, timespan="day"
            )
        except Exception as e:
        ...
        mismatched_dates = []
        for i, (theta_date, polygon_date) in enumerate(zip(theta_dates, polygon_dates, strict=False)):
        ...
        for theta_idx, polygon_idx in zip(theta_df.index, polygon_df.index, strict=False):
        ...
        if failures:
            ...
            for row in comparison_data[:10]:
                try:
                    ...
                    print(
                        f"   {date_str:<12} ${row['theta_close']:<9.2f} ${row['poly_close']:<9.2f} {'N/A':<9} Yahoo error"
                    )
        ...
        print(
            f"      Max differences: open=${max_diff['open']:.4f}, high=${max_diff['high']:.4f}, "
        ...
        for i, (theta_ts, polygon_ts) in enumerate(zip(theta_df.index, polygon_df.index, strict=False)):
        ...
        if "volume" in theta_df.columns and "volume" in polygon_df.columns:
            ...
            print(
                f"   ✓ Volume data present (Theta: {theta_zero_vol} zero days, Polygon: {polygon_zero_vol} zero days)"
            )
...
class TestIntradayDataComparison:
    ...
    def test_theta_interval_matches_minute_resample(self, interval, resample_rule, theta_minute_df):
        ...
        if bar_diff > 1:
            ...
            pytest.fail(
                f"CRITICAL: Bar count diff {bar_diff} > 1. Theta={len(theta_df)}, Resampled={len(resampled_df)}"
            )
        ...
        timestamp_mismatches = []
        for i, (theta_ts, resampled_ts) in enumerate(zip(theta_df.index, resampled_df.index, strict=False)):
        ...
        for theta_ts, resampled_ts in zip(theta_df.index, resampled_df.index, strict=False):
        ...
        if price_failures:
            ...
            for failure in price_failures[:10]:
                print(
                    f"      {failure['timestamp']} {failure['field']}: Theta=${failure['theta']:.4f}, "
        ...
        print("   ✓ All prices within $0.01 tolerance")
        print(
            f"      Max differences: open=${max_diff['open']:.4f}, high=${max_diff['high']:.4f}, "
            f"low=${max_diff['low']:.4f}, close=${max_diff['close']:.4f}"
        )
        ...
        print("    Timestamps: PERFECT MATCH (0 shifts)")
        print("    Prices: ALL within $0.01 (penny)")
