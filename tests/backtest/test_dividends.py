...
class DividendTestStrategy(Strategy):
    ...
    def on_trading_iteration(self):
        ...
        self.log_message(
            f"Date: {current_dt.strftime('%Y-%m-%d')}, Cash: ${current_cash:.2f}, "
...
class TestDividends:
    ...
    def _run_dividend_test(self, data_source_class, **data_source_kwargs):
        ...
        data_source = data_source_class(
            datetime_start=backtesting_start, datetime_end=backtesting_end, **data_source_kwargs
        )
    ...
    def _verify_dividend_test_results(self, strategy, data_source_name):
        ...
        for entry in strategy.cash_tracking:
            print(
                f"  {entry['datetime'].strftime('%Y-%m-%d')}: Cash=${entry['cash']:.2f}, Portfolio=${entry['portfolio_value']:.2f}"
            )
        ...
        post_purchase_cash = [
            entry["cash"] for entry in strategy.cash_tracking if entry["cash"] < strategy.initial_cash * 0.95
        ]
        ...
        cash_after_purchase = (
            ...
            else (strategy.cash_after_purchase if strategy.cash_after_purchase is not None else strategy.final_cash)
        )
        ...
        if final_cash > cash_after_purchase + dividend_threshold:
            print(
                f"  ✓ {data_source_name}: Dividends appear to be handled (cash increased by ${final_cash - cash_after_purchase:.2f})"
            )
            return True
        else:
            print(
                f"  ✗ {data_source_name}: No dividend increase detected (cash change: ${final_cash - cash_after_purchase:.2f})"
            )
            return False
    ...
    def test_polygon_dividends(self):
        ...
        # Run the backtest
        results, strategy = self._run_dividend_test(PolygonDataBacktesting, api_key=POLYGON_API_KEY)
    ...
    def test_compare_yahoo_vs_polygon_dividends(self):
        ...
        # Test Polygon
        polygon_results, polygon_strategy = self._run_dividend_test(PolygonDataBacktesting, api_key=POLYGON_API_KEY)
        ...
        if not yahoo_dividend_handled and not polygon_dividend_handled:
            print("  🚨 ISSUE: Neither data source appears to handle dividends!")
        elif yahoo_dividend_handled != polygon_dividend_handled:
            print("  ⚠️  WARNING: Inconsistent dividend handling between data sources!")
        else:
            print("  ✅ Both data sources handle dividends consistently")
