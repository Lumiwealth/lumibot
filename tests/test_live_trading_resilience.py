...
class MockDataSource:
    ...
    def get_datetime(self, adjust_for_delay=False):
        return datetime.datetime.now().astimezone(LUMIBOT_DEFAULT_PYTZ)
...
class TestLiveTradingResilience(unittest.TestCase):
    def test_live_trading_resilience_is_working(self):
        ...
        self.assertGreater(elapsed_time, 2.5, f"Expected trader to run for ~3s, but only ran {elapsed_time:.1f}s")
