...
class TestCSVErrorHandler:
    ...
    def test_error_details_normalization(self):
        ...
        normalized = handler._normalize_error_details('Error with "request_id":"abc123" in response', "TEST_ERROR")
        assert '"request_id":"<REDACTED>"' in normalized
        assert "abc123" not in normalized

        # Test timestamp normalization
        normalized = handler._normalize_error_details("Error at 2023-01-01T10:30:00", "TEST_ERROR")
            ...
            def log_errors(thread_id):
                for i in range(10):
                    record = logging.LogRecord(
                        ...
                        pathname="test.py",
                        lineno=i,
                        msg=f"Error {i} from thread {thread_id}",
                        args=(),
                        exc_info=None,
                        func="test_func",
                    )
