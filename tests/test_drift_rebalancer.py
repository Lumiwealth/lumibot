...
from lumibot.components.drift_rebalancer_logic import (
    DriftCalculationLogic,
    DriftOrderLogic,
    DriftRebalancerLogic,
    DriftType,
    get_last_price_or_raise,
)
...
class EvenOddDriftRebalancer(DriftRebalancer):
    ...
    def on_trading_iteration(self) -> None:
        ...
        if self.cash < 0:
            self.logger.error(f"Negative cash: {self.cash} but DriftRebalancer does not support margin yet.")
        ...
        if is_even_day:
            ...
            self.portfolio_weights = [{"base_asset": tlt_asset, "weight": Decimal("1.0")}]
...
class MockStrategyWithDriftCalculationLogic(Strategy):
    ...
    def get_last_price(self, asset: Asset | str, quote=None, exchange=None) -> float | Decimal | None:
...
class TestDriftCalculationLogic:
    ...
    def test_calculate_absolute_drift(self, mocker):
        strategy = MockStrategyWithDriftCalculationLogic(
            broker=self.backtesting_broker, drift_threshold=Decimal("0.05"), drift_type=DriftType.ABSOLUTE
        )
        portfolio_weights = [
            {"base_asset": Asset(symbol="AAPL", asset_type="stock"), "weight": Decimal("0.5")},
            {"base_asset": Asset(symbol="GOOGL", asset_type="stock"), "weight": Decimal("0.3")},
    ...
    def test_calculate_relative_drift(self, mocker):
        strategy = MockStrategyWithDriftCalculationLogic(
            broker=self.backtesting_broker, drift_threshold=Decimal("0.20"), drift_type=DriftType.RELATIVE
        )
        ...
        pd.testing.assert_series_equal(
            df["current_weight"], pd.Series([Decimal("0.4"), Decimal("0.4"), Decimal("0.2")]), check_names=False
        )
        ...
        pd.testing.assert_series_equal(
            df["drift"],
            pd.Series(
                [Decimal("0.3333333333333333333333333333"), Decimal("-0.3333333333333333333333333333"), Decimal("-1.0")]
            ),
            check_names=False,
        )
    ...
    def test_drift_is_negative_one_when_we_have_a_position_and_the_target_weights_says_to_not_have_it(self, mocker):
        ...
        pd.testing.assert_series_equal(
            df["drift"],
            pd.Series(
                [Decimal("0.0454545454545454545454545455"), Decimal("-0.0030303030303030303030303030"), Decimal("-1")]
            ),
            check_names=False,
        )
    ...
    def test_drift_is_negative_one_when_we_have_none_of_an_asset_and_target_weights_says_we_should_short_some(
        self, mocker
    ):
        strategy = MockStrategyWithDriftCalculationLogic(
    ...
    def test_shorting_more_when_price_goes_up_short_something(self, mocker):
        ...
        df = strategy.drift_rebalancer_logic.calculate(portfolio_weights=portfolio_weights)
        assert df["current_weight"].tolist() == [
            Decimal("-0.5789473684210526315789473684"),
    ...
    def test_calculate_absolute_drift_when_we_want_a_100_percent_short_position_and_cash_in_target_weights(
        self, mocker
    ):
        strategy = MockStrategyWithDriftCalculationLogic(
            broker=self.backtesting_broker, drift_threshold=Decimal("0.05"), drift_type=DriftType.ABSOLUTE
    ...
    def test_calculate_relative_drift_when_we_want_a_100_percent_short_position(self, mocker):
        strategy = MockStrategyWithDriftCalculationLogic(
            broker=self.backtesting_broker, drift_threshold=Decimal("0.05"), drift_type=DriftType.RELATIVE
        )
...
class TestDriftOrderLogic:
    ...
    def test_selling_everything_with_limit_orders(self):
        strategy = MockStrategyWithOrderLogic(broker=self.backtesting_broker, order_type=Order.OrderType.LIMIT)
        df = pd.DataFrame(
            {
                "symbol": ["AAPL"],
    ...
    def test_selling_everything_with_market_orders(self):
        strategy = MockStrategyWithOrderLogic(broker=self.backtesting_broker, order_type=Order.OrderType.MARKET)
        df = pd.DataFrame(
            {
                "symbol": ["AAPL"],
                "base_asset": [Asset("AAPL", "stock")],
    ...
    def test_selling_small_short_position_creates_and_order_when_shorting_is_enabled(self):
        strategy = MockStrategyWithOrderLogic(
            broker=self.backtesting_broker, order_type=Order.OrderType.LIMIT, shorting=True
        )
        df = pd.DataFrame(
            {
                "symbol": ["AAPL"],
    ...
    def test_selling_small_short_position_doesnt_create_order_when_shorting_is_disabled(self):
        strategy = MockStrategyWithOrderLogic(
            broker=self.backtesting_broker, order_type=Order.OrderType.MARKET, shorting=False
        )
        df = pd.DataFrame(
            {
                "symbol": ["AAPL"],
    ...
    def test_selling_a_100_percent_short_position_creates_an_order_when_shorting_is_enabled(self):
        strategy = MockStrategyWithOrderLogic(self.backtesting_broker, order_type=Order.OrderType.LIMIT, shorting=True)
        df = pd.DataFrame(
            [
                {
                    "symbol": "AAPL",
    ...
    def test_calculate_limit_price_when_selling(self):
        ...
        strategy.order_logic = DriftOrderLogic(strategy=strategy, acceptable_slippage=Decimal("0.005"))
        strategy.order_logic.rebalance(drift_df=df)
    ...
    def test_buying_whole_shares(self):
        strategy = MockStrategyWithOrderLogic(
            broker=self.backtesting_broker, order_type=Order.OrderType.LIMIT, fractional_shares=False
        )
        df = pd.DataFrame(
            {
                "symbol": ["AAPL"],
    ...
    def test_buying_fractional_shares(self):
        strategy = MockStrategyWithOrderLogic(
            broker=self.backtesting_broker, order_type=Order.OrderType.LIMIT, fractional_shares=True
        )
    ...
    def test_calculate_trading_costs_with_percent_fees(self):
        ...
        total_cost = strategy.order_logic.calculate_trading_costs(
            quantity, price, TradingFee(percent_fee=Decimal("0.0025"))
        )
    ...
    def test_calculate_trading_costs_with_flat_fees(self):
        ...
        total_cost = strategy.order_logic.calculate_trading_costs(quantity, price, TradingFee(flat_fee=Decimal("1.00")))
    ...
    def test_calculate_trading_costs_with_both_fee_types(self):
        ...
        total_cost = strategy.order_logic.calculate_trading_costs(
            quantity, price, [TradingFee(flat_fee=Decimal("1.00")), TradingFee(percent_fee=Decimal("0.0025"))]
        )
    ...
    def test_adjust_quantity_for_fees_buy_affordable(self):
        ...
        adjusted_quantity = strategy.order_logic.adjust_quantity_for_fees(
            desired_quantity, last_price, side, TradingFee(percent_fee=Decimal("0.0025")), capital_available
        )
    ...
    def test_get_current_cash_position(self, mocker):
        ...
        mocker.patch.object(type(strategy), "cash", new_callable=PropertyMock(return_value=Decimal("15000.50")))

        cash_position = strategy.order_logic.get_current_cash_position()
        assert cash_position == Decimal("15000.50")

        mocker.patch.object(type(strategy), "cash", new_callable=PropertyMock(return_value=Decimal("15000.666666")))
...
class TestDriftRebalancer:
    ...
    def test_classic_60_40(self, pandas_data_fixture):
        parameters = {
            ...
            "fill_sleeptime": 15,
            "portfolio_weights": [
                {"base_asset": Asset(symbol="SPY", asset_type="stock"), "weight": Decimal("0.6")},
                {"base_asset": Asset(symbol="TLT", asset_type="stock"), "weight": Decimal("0.4")},
            ],
            "shorting": False,
        }
    ...
    def test_crypto_50_50_with_yahoo(self):
        parameters = {
            ...
            "fill_sleeptime": 15,
            "portfolio_weights": [
                {"base_asset": Asset(symbol="BTC-USD", asset_type="stock"), "weight": Decimal("0.5")},
                {"base_asset": Asset(symbol="ETH-USD", asset_type="stock"), "weight": Decimal("0.5")},
            ],
            "shorting": False,
            "fractional_shares": True,
        }
    ...
    def test_crypto_50_50_with_polygon(self):
        parameters = {
            ...
            "fill_sleeptime": 15,
            "portfolio_weights": [
                {"base_asset": Asset(symbol="BTC", asset_type="crypto"), "weight": Decimal("0.5")},
                {"base_asset": Asset(symbol="ETH", asset_type="crypto"), "weight": Decimal("0.5")},
            ],
            "shorting": False,
            "fractional_shares": True,
        }
...
class TestDriftRebalancerOptions:
    ...
    def test_options_limit_price_calculation(self):
        ...
        buy_limit = strategy.order_logic.calculate_limit_price(
            last_price=Decimal("25.00"), side="buy", asset=option_asset
        )
        Decimal("25.00") * (1 + Decimal("0.005"))  # Add 0.5% slippage
        ...
        sell_limit = strategy.order_logic.calculate_limit_price(
            last_price=Decimal("25.00"), side="sell", asset=option_asset
        )
        Decimal("25.00") * (1 - Decimal("0.005"))  # Subtract 0.5% slippage
