    ...
    def get_from_endpoint(
        ...
        while (not allow_fail) or first_run:
            try:
                ...
                if response.status_code == 200:
                    ...
                    if not silent:
                        if (not allow_fail) and first_run:
                            logging.warning(colored(f"error: {description} endpoint not found. Retrying...", "yellow"))
                            time.sleep(1)
                            ...
                            logging.error(colored(f"error: {description} endpoint not found.", "yellow"))
                    ...
                    if not silent:
                        if (not allow_fail) and first_run:
                            logging.warning(colored(f"error: Task '{description}' Failed. Status code: {response.status_code}, Response: {response.text} Retrying...", "yellow"))
                            time.sleep(1)
                            ...
                            logging.error(colored(f"error: Task '{description}' Failed. Status code: {response.status_code}, Response: {response.text}", "yellow"))
                ...
                if not silent:
                    if (not allow_fail) and first_run:
                        logging.warning(colored(f"error: {description}. Retrying...", "yellow"))
                        time.sleep(1)
