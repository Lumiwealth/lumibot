...
class TestTradovateImports:
    ...
    def test_tradovate_exception_import(self):
        ...
        TradovateAPIError = tradovate_module.TradovateAPIError
...
class TestTradovateBroker:
    ...
    def test_broker_has_required_methods(self):
        ...
        essential_methods = ["__init__", "_get_tokens", "_get_account_info", "_get_user_info"]
    ...
    def test_broker_config_validation(self):
        ...
        with patch.object(Tradovate, "_get_tokens") as mock_get_tokens, patch.object(
            Tradovate, "_get_account_info"
        ) as mock_get_account_info, patch.object(Tradovate, "_get_user_info") as mock_get_user_info:
            ...
            mock_get_account_info.return_value = {"accountSpec": "fake_account_spec", "accountId": 123456}
            ...
            try:
                ...
                raise AssertionError(f"Broker initialization failed with mocked API: {e}") from e
    ...
    def test_broker_handles_missing_credentials(self):
        ...
        with patch("requests.request") as mock_request:
            ...
            try:
                ...
                raise AssertionError("Broker should have failed with empty config")
            except Exception as e:
                # Should fail due to authentication/credentials issue
                error_msg = str(e).lower()
                # The error should be about authentication or bad request
                assert any(
                    keyword in error_msg for keyword in ["authentication", "failed", "bad request", "credentials"]
                )
...
class TestTradovateSymbolConversion:
    ...
    def test_continuous_futures_symbol_resolution(self):
        ...
        print("✅ Symbol conversion test passed:")
        print(f"   MES -> {mes_resolved}")
        print(f"   ES -> {es_resolved}")
        print("   MESU25 -> MESU25 (specific contract, unchanged)")
    ...
    def test_order_symbol_extraction(self):
        ...
        # Create order
        order = Order(strategy=mock_strategy, asset=asset, quantity=1, side="buy", order_type=Order.OrderType.MARKET)
...
class TestTradovateSymbolResolution:
    ...
    def test_tradovate_vs_standard_symbol_difference(self):
        ...
        standard_symbol = asset.resolve_continuous_futures_contract(reference_date=reference_date, year_digits=2)
        tradovate_expected = asset.resolve_continuous_futures_contract(reference_date=reference_date, year_digits=1)
...
class TestTradovateLifecycle:
    ...
    def _make_broker(self):
        ...
        with patch.object(Tradovate, "_get_tokens", return_value=tokens), patch.object(
            Tradovate, "_get_account_info", return_value=account_info
    ...
    def test_submit_order_emits_new_event(self):
        ...
        with patch.object(broker, "_request", return_value=mock_response), patch.object(
    ...
    def test_do_polling_dispatches_fill_event(self):
        ...
        with patch.object(broker, "sync_positions", return_value=None), patch.object(
            broker, "_pull_broker_all_orders", return_value=[{"id": "321"}]
        ), patch.object(broker, "_parse_broker_order", return_value=filled_order), patch.object(
            broker, "_extract_fill_details", return_value=(100.0, 2)
        ):
            broker.do_polling()
    ...
    def test_do_polling_dispatches_new_for_active_order(self):
        ...
        with patch.object(broker, "sync_positions", return_value=None), patch.object(
            broker, "_pull_broker_all_orders", return_value=[{"id": "999"}]
        ), patch.object(broker, "_parse_broker_order", return_value=active_order), patch.object(
            broker, "_extract_fill_details", return_value=(None, None)
        ):
            broker.do_polling()
    ...
    def test_do_polling_skips_new_for_closed_order_even_after_startup(self):
        ...
        with patch.object(broker, "sync_positions", return_value=None), patch.object(
            broker, "_pull_broker_all_orders", return_value=[{"id": "777"}]
        ), patch.object(broker, "_parse_broker_order", return_value=closed_order), patch.object(
            broker, "_extract_fill_details", return_value=(100.0, 1)
        ):
            broker.do_polling()
    ...
    def test_missing_order_reconciles_to_fill_instead_of_cancel(self):
        ...
        with patch.object(broker, "sync_positions", return_value=None), patch.object(
            broker, "_pull_broker_all_orders", return_value=[]
        ), patch.object(broker, "get_all_orders", return_value=[missing_order]), patch.object(
            broker, "_fetch_recent_fill_details", return_value=(6788.5, 1)
    ...
    def test_cancel_open_orders_prunes_stale_locals(self):
        ...
        with patch.object(
            broker, "_refresh_active_identifiers_snapshot", return_value={"222"}
        ) as mock_refresh, patch.object(broker, "cancel_orders") as mock_cancel:
    ...
    def test_cancel_open_orders_refreshes_cache_when_missing(self):
        ...
        with patch.object(
            broker, "_refresh_active_identifiers_snapshot", return_value={"333"}
        ) as mock_refresh, patch.object(broker, "cancel_orders") as mock_cancel:
            broker.cancel_open_orders("Strategy")
...
class TestTradovateTokenRenewal:
    ...
    def test_token_renewal_on_expiry(self):
        ...
        with patch.object(Tradovate, "_get_tokens") as mock_get_tokens, patch.object(
            ...
            mock_get_account_info.return_value = {"accountSpec": "TEST_ACCOUNT", "accountId": 12345}
