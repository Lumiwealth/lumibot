...
def test_data_polars_row_count_parity():
    ...
    print(f"Polars rows: {len(df_polars)}")
    print(f"\nPandas timestamps:")
    if isinstance(df_pandas, pd.DataFrame):
        print(df_pandas.index.tolist() if hasattr(df_pandas, 'index') else df_pandas['datetime'].tolist())
    print(f"\nPolars timestamps:")
    if isinstance(df_polars, pl.DataFrame):
        print(df_polars['datetime'].to_list())
    elif isinstance(df_polars, pd.DataFrame):
        print(df_polars.index.tolist() if hasattr(df_polars, 'index') else df_polars['datetime'].tolist())
...
def test_data_polars_timeshift_timedelta():
    ...
    print(f"\n=== Timedelta Timeshift Test ===")
    print(f"Timeshift: {timeshift_td}")
    print(f"Pandas rows: {len(df_pandas)}")
    print(f"Polars rows: {len(df_polars)}")
