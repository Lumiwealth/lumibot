...
# pyright: reportInvalidTypeForm=false, reportUnknownLambdaType=false
...
BracketMeta: TypeAlias = dict[str, Any]  # noqa: UP040
PriceValue: TypeAlias = float | int | None  # noqa: UP040
...
_SIGNALR_MODULE: Any = _LazyModule("signalrcore.hub_connection_builder")
try:
    _signalr_available = find_spec("signalrcore.hub_connection_builder") is not None
except ModuleNotFoundError:
    _signalr_available = False
SIGNALR_AVAILABLE: bool = _signalr_available


def _hub_connection_builder_class() -> Any:
...
def _iter_error_messages(errors: Any) -> list[str]:
    ...
    for err_list in errors.values():
        if isinstance(err_list, list):
            messages.extend(str(err) for err in err_list)
        else:
            messages.append(str(err_list))
    return messages
...
class ProjectXAuth:
    ...
    @staticmethod
    def get_auth_token(config: Config) -> str | None:
        ...
        auth_resp = _json_dict(auth_resp)
...
class ProjectXStreaming:
    ...
    def __init__(self, config: Config, token: str, account_id: int | None = None) -> None:
        if not SIGNALR_AVAILABLE:
            raise ImportError("signalrcore library is required for streaming functionality")

        self.firm = str(config.get("firm", ""))
        self.token = token
        ...
        if streaming_base_url:
            self.base_url = str(streaming_base_url)
        else:
            base_url = config.get("base_url")
            if not base_url:
        ...
        # Event handlers
        self.on_account_update: Callable[[Any], Any] | None = None
        self.on_order_update: Callable[[Any], Any] | None = None
        self.on_position_update: Callable[[Any], Any] | None = None
        self.on_trade_update: Callable[[Any], Any] | None = None
    ...
    def start_user_hub(self) -> bool:
        ...
        try:
            ...
            connection.on_open(lambda: self._on_user_hub_open())
            connection.on_close(lambda: self._on_user_hub_close())
            connection.on_error(lambda data: self._on_user_hub_error(data))
            ...
            while time.time() - start_time < connection_timeout:
                if hasattr(connection, "transport") and connection.transport:
                    if (
                        hasattr(connection.transport, "state")
                        and connection.transport.state == "connected"
                    ):
...
class ProjectX:
    ...
    def __init__(self, config: Config, token: str) -> None:
        ...
        self.headers: dict[str, str] = {"Authorization": f"Bearer {token}", "Content-Type": "application/json"}

        self.logger = get_logger(f"ProjectXClient_{self.firm}")

    def get_streaming_client(self, account_id: int | None = None) -> ProjectXStreaming:
        """Get streaming client instance"""
        return ProjectXStreaming(self.config, self.token, account_id)

    def account_search(self, only_active_accounts: bool = True) -> JsonDict:
    ...
    def position_search_open(self, account_id: int) -> JsonDict:
    ...
    def order_search(self, account_id: int, start_datetime: str, end_datetime: str | None = None) -> JsonDict:
    ...
    def trade_search(self, account_id: int, start_timestamp: str, end_timestamp: str | None = None) -> JsonDict:
    ...
    def order_place(
        ...
        def _clean(d: JsonDict) -> JsonDict:
        ...
        try:
            ...
            error_messages = _iter_error_messages(errors)
            needs_wrapper = False
            if isinstance(errors, dict):
                if "request" in errors or any("The request field is required" in err for err in error_messages):
                    needs_wrapper = True
            # Also if size conversion failed, we retry after forcing int already and potentially wrapping
            size_error = any("$.size" in err for err in error_messages) if errors else False
            ...
            if needs_wrapper or size_error:
                ...
                try:
                    retry_data = _json_dict(retry_resp.json())
                except ValueError:
    ...
    def order_cancel(self, account_id: int, order_id: int) -> JsonDict:
        """Cancel an order"""
        url = f"{self.base_url}api/order/cancel"
        payload: JsonDict = {"accountId": account_id, "orderId": order_id}
        try:
            response = requests.post(url, headers=self.headers, json=payload, timeout=10)
            try:
                data = _json_dict(response.json())
    ...
    def contract_search(self, search_text: str, live: bool = False) -> JsonDict:
        ...
        try:
            response = requests.post(url, headers=self.headers, json=payload)
            return _json_dict(response.json())
        except requests.exceptions.RequestException as e:
            self.logger.error(f"Error: {e}")
            return {"success": False, "error": str(e)}

    def contract_search_id(self, contract_id: str) -> JsonDict:
...
class ProjectXClient:
    ...
    def __init__(self, config: Config) -> None:
        ...
        # Streaming client
        self.streaming: ProjectXStreaming | None = None

        # Cache for contract details to reduce API calls
        self._contract_cache: dict[str, JsonDict] = {}

        # Enhanced caching
        self._account_cache: list[JsonDict] | None = None
        self._account_cache_time = 0.0
        self._positions_cache: list[JsonDict] | None = None
        self._positions_cache_time = 0.0
        self._orders_cache: list[JsonDict] | None = None
        self._orders_cache_time = 0.0
        self._cache_ttl = 5.0

    def get_accounts(self) -> list[JsonDict]:
        ...
        if response and response.get("success"):
            accounts = _json_list(response.get("accounts", []))
    ...
    def get_preferred_account_id(self) -> int:
        ...
        if account_id is None:
            raise Exception("Selected account is missing an id")
    ...
    def get_account_balance(self, account_id: int) -> JsonDict:
    ...
    def get_positions(self, account_id: int) -> list[JsonDict]:
        ...
        if response and response.get("success"):
            positions = _json_list(response.get("positions", []))
            # Update cache
    ...
    def get_orders(self, account_id: int, start_date: str | None = None, end_date: str | None = None) -> list[JsonDict]:
        ...
        if response and response.get("success"):
            orders = _json_list(response.get("orders", []))
            # Update cache
    ...
    def place_order(
        self, account_id: int, contract_id: str, side: str, quantity: int, order_type: int, price: float | None = None
    ) -> JsonDict:
    ...
    def search_contracts(self, search_text: str) -> list[JsonDict]:
        """Search for contracts"""
        response = self.api.contract_search(search_text)
        if response and response.get("success"):
            return _json_list(response.get("contracts", []))
        else:
    ...
    def contract_search(self, search_text: str) -> JsonDict:
    ...
    def get_contract_details(self, contract_id: str) -> JsonDict:
        ...
        if response and response.get("success"):
            direct_contract = response.get("contract")
            if isinstance(direct_contract, dict):
                contract_details = cast(JsonDict, direct_contract)
                self._contract_cache[contract_id] = contract_details
                return contract_details

            contracts = _json_list(response.get("contracts", []))
            if contracts:
        ...
        try:
            ...
            try:
                ...
                if contracts:
                    ...
                    contract_id = str(
                        to_search[0].get("contractId") or to_search[0].get("id") or to_search[0].get("symbol") or ""
                    )
                    self.logger.info(f"✅ Using active contract: {contract_id}")
                    return contract_id

                else:
                    self.logger.warning("⚠️ No contracts found via API search, falling back to search Asset logic")
    ...
    def get_contract_tick_size(self, contract_id: str) -> float:
        ...
        try:
            ...
            tick_size = contract_details.get("tickSize", contract_details.get("TickSize", 0.25))
    ...
    def round_to_tick_size(self, price: float | None, tick_size: float | None) -> float | None:
...
def _to_utc_iso(dt_or_str: str | datetime, is_est: bool = True) -> str:
    """
...
def create_bracket_meta(tp_price: PriceValue, sl_price: PriceValue) -> BracketMeta:
...
def normalize_bracket_entry_tag(tag: str | None) -> tuple[str | None, str | None]:
...
def build_unique_order_tag(order: Any) -> str | None:
    ...
    try:
        ...
        if not getattr(order, "tag", None):
            ...
            try:
                ...
                if strategy:
                    strat_name = strategy if isinstance(strategy, str) else getattr(strategy, "name", "")
        ...
        pass
    return cast(str | None, getattr(order, "tag", None))


def select_effective_prices(order: Any, client: Any, tick_size: float | None) -> tuple[float | None, float | None]:
    ...
    if getattr(order, "take_profit_price", None) is not None:
        try:
            ...
            logger_obj = getattr(getattr(order, "strategy", None), "logger", None)
            if logger_obj:
                logger_obj.warning(
    ...
    secondary_limit_price = getattr(order, "secondary_limit_price", None)
    limit_attr = getattr(order, "limit_price", None)
    if secondary_limit_price is not None:
        try:
            limit_price = client.round_to_tick_size(secondary_limit_price, tick_size)
        except Exception:
            if limit_attr is not None:
                try:
                    limit_price = client.round_to_tick_size(limit_attr, tick_size)
                except Exception:
                    pass
    elif limit_attr is not None:
        try:
            limit_price = client.round_to_tick_size(limit_attr, tick_size)
        except Exception:
            pass
    ...
    secondary_stop_price = getattr(order, "secondary_stop_price", None)
    stop_attr = getattr(order, "stop_price", None)
    if secondary_stop_price is not None:
        try:
            stop_price = client.round_to_tick_size(secondary_stop_price, tick_size)
        except Exception:
            if stop_attr is not None:
                try:
                    stop_price = client.round_to_tick_size(stop_attr, tick_size)
                except Exception:
                    pass
    elif stop_attr is not None:
        try:
            stop_price = client.round_to_tick_size(stop_attr, tick_size)
        except Exception:
            pass
...
def early_store_bracket_meta(
    store: MutableMapping[str, BracketMeta] | None, temp_key: str, meta: BracketMeta, logger: Any | None = None
) -> None:
...
def restore_bracket_meta_if_needed(
    order: Any, cache: Mapping[Any, Any] | None, meta_map: Mapping[Any, BracketMeta] | None, logger: Any | None = None
) -> bool:
...
def should_spawn_bracket_children(meta: BracketMeta | None, parent: Any) -> tuple[bool, str]:
...
def build_bracket_child_spec(parent: Any, kind: str, price: float, base_tag: str) -> JsonDict:
    ...
    """
    side = "sell" if str(getattr(parent, "side", "")).lower() == "buy" else "buy"
    if kind == "tp":
        order_type = "limit"
