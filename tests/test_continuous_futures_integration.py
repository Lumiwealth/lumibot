...
class TestContinuousFuturesIntegration(unittest.TestCase):
    ...
    def test_asset_class_consistency(self):
        ...
        for symbol in test_symbols:
            with self.subTest(symbol=symbol):
                ...
                primary_contract = asset.resolve_continuous_futures_contract(reference_date=self.test_date)
                potential_contracts = asset.get_potential_futures_contracts(reference_date=self.test_date)
    ...
    def test_databento_integration_consistency(self):
        ...
        databento_result = _format_futures_symbol_for_databento(continuous_asset, reference_date=self.test_date)

        # Get result from Asset class directly
        asset_result = continuous_asset.resolve_continuous_futures_contract(reference_date=self.test_date)
    ...
    def test_multi_component_consistency(self):
        ...
        asset_primary = asset.resolve_continuous_futures_contract(reference_date=self.test_date)
        asset_potential = asset.get_potential_futures_contracts(reference_date=self.test_date)
        databento_result = _format_futures_symbol_for_databento(asset, reference_date=self.test_date)
    ...
    def test_contract_format_standardization(self):
        ...
        for symbol, asset_pattern, databento_pattern in test_cases:
            with self.subTest(symbol=symbol):
                ...
                asset_result = asset.resolve_continuous_futures_contract(reference_date=self.test_date)
    ...
    def test_no_duplicate_logic_verification(self):
        ...
        with patch.object(Asset, "resolve_continuous_futures_contract", return_value="MOCKED_CONTRACT") as mock_method:
    ...
    def test_quarterly_preference_consistency(self):
        ...
        for test_date, expected_asset_month, expected_databento_month in quarterly_test_cases:
            with self.subTest(date=test_date):
                ...
                # Test Asset class
                asset_result = asset.resolve_continuous_futures_contract(reference_date=test_date)
                asset_month_code = asset_result[-3]  # Third from end

                # Test DataBento integration
                databento_result = _format_futures_symbol_for_databento(asset, reference_date=test_date)
    ...
    def test_centralization_verification(self):
        ...
        # by verifying it uses Asset class methods
        with patch.object(
            Asset, "resolve_continuous_futures_contract", return_value="CENTRALIZED_RESULT"
        ) as mock_method:
            result = _format_futures_symbol_for_databento(asset)
