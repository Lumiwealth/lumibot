...
class TestCash(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.data_source = YahooDataBacktesting(datetime_start=self.start_date, datetime_end=self.end_date)

        self.broker = BacktestingBroker(self.data_source)

        # Create strategy
        self.strategy = CashTestStrategy(broker=self.broker, budget=100000, name="CashTestStrategy")
    ...
    def test_initial_cash_equals_budget(self):
        ...
        self.assertEqual(property_cash, expected_budget, f"Initial self.cash should equal budget: {expected_budget}")
        self.assertEqual(method_cash, expected_budget, f"Initial get_cash() should equal budget: {expected_budget}")
    ...
    def test_no_private_cash_variable(self):
        ...
        self.assertFalse(
            hasattr(self.strategy, "_cash"), "Strategy should not have a _cash private variable to avoid sync issues"
        )
    ...
    def test_cash_after_position_creation(self):
        ...
        self.assertEqual(
            property_cash, new_cash_amount, f"self.cash should equal {new_cash_amount} after _set_cash_position"
        )
        self.assertEqual(
            method_cash, new_cash_amount, f"get_cash() should equal {new_cash_amount} after _set_cash_position"
        )
        self.assertEqual(
            property_cash, method_cash, "self.cash and get_cash() should remain consistent after position change"
        )
    ...
    def test_cash_after_trade_simulation(self):
        ...
        self.assertEqual(property_cash, new_cash, f"self.cash should equal {new_cash} after simulated trade")
        self.assertEqual(method_cash, new_cash, f"get_cash() should equal {new_cash} after simulated trade")
        self.assertEqual(property_cash, method_cash, "self.cash and get_cash() should remain consistent after trade")
    ...
    def test_cash_multiple_position_updates(self):
        ...
        for amount in test_amounts:
            with self.subTest(amount=amount):
                ...
                self.assertEqual(property_cash, amount, f"self.cash should equal {amount}")
                self.assertEqual(method_cash, amount, f"get_cash() should equal {amount}")
                self.assertEqual(property_cash, method_cash, f"Both methods should be consistent for amount {amount}")
    ...
    def test_cash_with_decimal_values(self):
        ...
        self.assertAlmostEqual(
            property_cash, decimal_amount, places=2, msg="self.cash should handle decimal values correctly"
        )
        self.assertAlmostEqual(
            method_cash, decimal_amount, places=2, msg="get_cash() should handle decimal values correctly"
        )
        self.assertEqual(property_cash, method_cash, "Both should return same decimal value consistently")
        ...
        for price in test_prices:
            with self.subTest(price=price):
                try:
                    ...
                    self.assertEqual(shares, expected_shares, f"Share calculation incorrect for price {price}")
    ...
    def test_strategy_mathematical_operations(self):
        ...
        try:
            ...
            self.assertIsNotNone(self.strategy.last_available_cash, "Strategy should have stored available_cash")
            self.assertIsInstance(self.strategy.last_shares, int, "Strategy should have calculated integer shares")
    ...
    def test_multiple_get_cash_calls(self):
        ...
        for i, value in enumerate(cash_values):
            self.assertEqual(
                value, first_value, f"get_cash() call {i + 1} returned different value: {value} vs {first_value}"
            )
    ...
    def test_backtest_setup_cash_handling(self):
        ...
        try:
            ...
            data_source = YahooDataBacktesting(datetime_start=start_date, datetime_end=end_date)

            broker = BacktestingBroker(data_source)

            strategy = CashTestStrategy(broker=broker, budget=100000, name="BacktestCashTest")
    ...
    def test_various_budget_amounts(self):
        ...
        for budget in budgets:
            with self.subTest(budget=budget):
                ...
                strategy = CashTestStrategy(broker=broker, budget=budget, name=f"CashTestStrategy_{budget}")

                # Test consistency
                property_cash = strategy.cash
                method_cash = strategy.get_cash()

                self.assertEqual(property_cash, budget, f"self.cash should equal budget {budget}")
                self.assertEqual(method_cash, budget, f"get_cash() should equal budget {budget}")
                self.assertEqual(property_cash, method_cash, f"Both methods should be consistent for budget {budget}")
