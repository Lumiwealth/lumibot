...
@pytest.mark.skipif(not os.environ.get("DATABENTO_API_KEY"), reason="DATABENTO_API_KEY environment variable not set")
...
def test_minute_bar_aggregation():
    ...
    data_source = DataBentoDataPolars(
        api_key=os.getenv("DATABENTO_API_KEY"), has_paid_subscription=True, enable_live_stream=True
    )
    ...
    bars = data_source.get_historical_prices(asset=asset, length=5, timestep="minute")
...
def test_long_time_periods():
    ...
    for length, description in test_cases:
        ...
        # Get bars
        bars = data_source.get_historical_prices(asset=asset, length=length, timestep="minute")
        ...
        if bars and len(bars) > 0:
            ...
            if latest_time:
                ...
                if lag_minutes <= 1.5:  # 90 seconds
                    ...
                    print(f"    Latest bar time: {latest_time}")
                    print("    This is the ACTUAL problem the bot sees!")
            else:
