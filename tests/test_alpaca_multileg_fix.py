...
class TestAlpacaMultiLegOrders:
    ...
    def setup_method(self):
        ...
        self.test_config = {"API_KEY": "test_api_key_multileg", "API_SECRET": "test_api_secret_multileg", "PAPER": True}

        # Create sample option assets for testing
        self.expiration = datetime.now() + timedelta(days=30)

        self.call_asset = Asset(
            symbol="SPY", asset_type=Asset.AssetType.OPTION, expiration=self.expiration, strike=450.0, right="call"
        )

        self.put_asset = Asset(
            symbol="SPY", asset_type=Asset.AssetType.OPTION, expiration=self.expiration, strike=440.0, right="put"
        )
    ...
    def test_multileg_order_has_required_fields(self, mock_trading_client):
        ...
        mock_trading_client.return_value = Mock()
        Alpaca(self.test_config, connect_stream=False)
        call_order = Order(
            strategy="test_strategy",
            asset=self.call_asset,
            quantity=1,
