...
6. Evaluate both legs with options_evaluate_market. Price with
   options_calculate_multileg_price. Require a net credit (negative signed price)
   and 0 < credit < {wing_width}.
7. Size so max loss (({wing_width} - credit) * 100 * contracts) is <= {max_risk_pct_display:.2f}
   percent of portfolio value, capped at {max_contracts} contracts.
8. Close when {profit_take_pct} percent of credit is captured, closing debit >=
...
9. After any orders_submit_multileg call, capture identifiers, call
    ...
    def on_trading_iteration(self):
        ...
        self.agents["credit_spread"].run(
            task_prompt=(
                ...
                "options_get_greeks before opening. After any submission, verify fills with "
                "orders_get_status and re-read positions."
            ),
            context={
...
if __name__ == "__main__":
    ...
    backtesting_start = datetime.fromisoformat(
        os.environ.get("BACKTESTING_START", (backtesting_end - timedelta(days=14)).date().isoformat())
    )
