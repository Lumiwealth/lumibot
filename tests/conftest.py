...
def pytest_configure(config):
    config.addinivalue_line(
        "markers", "ibkr: downloader-only IBKR tests that do not require Polygon or ThetaData credentials"
    )
...
def pytest_runtest_setup(item: pytest.Item):
    ...
    if missing:
        reason = "Skipping API test due to missing/placeholder credentials: " + ", ".join(missing)
        pytest.skip(reason)
