"""Yahoo backtesting data source implemented with polars.

This implementation:
1. Uses polars columnar storage
2. Avoids pandas conversions
3. Maintains compatibility with Pandas implementation
"""
...
class YahooDataBacktestingPolars(PolarsData):
    ...
    def _coerce_asset(self, asset: Asset | str) -> Asset:
        ...
        if symbol.endswith('=F') or symbol.endswith('=f'):
            return Asset(symbol.rstrip().upper(), asset_type=Asset.AssetType.FUTURE)
    ...
    def get_quote(
        ...
        **kwargs,
        ...
        if row_dict is None:
            logger.warning(
                "[YAHOO-QUOTE][POLARS][EMPTY] asset=%s timestep=%s current_dt=%s",
        ...
        logger.info(
            "[YAHOO-QUOTE][POLARS] asset=%s current_dt=%s price=%s bid=%s ask=%s mid=%s",
            getattr(asset, "symbol", asset),
            current_dt,
            price,
            bid,
