...
class TestContinuousFuturesResolution(unittest.TestCase):
    ...
    def test_month_progression_logic(self):
        ...
        for month, expected_month_code in test_cases:
            contract = asset.resolve_continuous_futures_contract(reference_date=datetime(2025, month, 15))
    ...
    def test_contract_format_validation(self):
        ...
        for contract in contracts:
            is_valid_format = (
                ("." not in contract and contract[-3:].isalnum())
                or ("." in contract and contract.split(".")[1].isalnum())
                or ("." not in contract and contract[-4:].isdigit())
            )
