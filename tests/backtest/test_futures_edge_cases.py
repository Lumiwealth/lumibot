...
class TestFuturesEdgeCases:
    ...
    def test_short_selling(self, datasource_cls):
        ...
        if len(strat.trades) >= 2:
            ...
            final_snapshot = next(
                (s for s in strat.snapshots if s["position_qty"] == 0 and s["iteration"] > sell_snapshot["iteration"]),
                strat.snapshots[-1],
            )

            margin_deposit = float(entry_snapshot["cash"]) - float(entry["cash_after"]) - fee_amount
            print(f"  Margin captured: ${margin_deposit:.2f}")
            assert pytest.approx(margin_deposit, abs=0.01) == 1300.0, (
                f"Expected $1,300 margin, got ${margin_deposit:.2f}"
            )
