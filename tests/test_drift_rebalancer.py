...
# class MockStrategyWithLimitOrderRebalancer(Strategy):
#
#     def __init__(self, *args, **kwargs):
#         super().__init__(*args, **kwargs)
#         self.orders = []
...
#         strategy = MockStrategyWithLimitOrderRebalancer(broker=self.backtesting_broker)
#         df = pd.DataFrame({
#             "symbol": ["AAPL"],
#             "is_quote_asset": False,
#             "current_quantity": [Decimal("10")],
...
class TestDriftOrderLogic:
    ...
    def test_selling_everything(self):
        strategy = MockStrategyWithOrderLogic(broker=self.backtesting_broker)
        df = pd.DataFrame({
            "symbol": ["AAPL"],
            "is_quote_asset": False,
            "current_quantity": [Decimal("10")],
    ...
    def test_selling_small_short_position_creates_and_order_when_shorting_is_enabled(self):
        ...
        strategy.order_logic = DriftOrderLogic(strategy=strategy, shorting=True)
        strategy.order_logic.rebalance(drift_df=df)
        assert len(strategy.orders) == 1
    ...
    def test_selling_small_short_position_doesnt_create_order_when_shorting_is_disabled(self):
        ...
        strategy.order_logic = DriftOrderLogic(strategy=strategy, shorting=False)
        strategy.order_logic.rebalance(drift_df=df)
        assert len(strategy.orders) == 0
