...
def main():
    ...
    results["thetadata_nocache"] = profile_backtest(
        ThetaDataBacktesting, "ThetaData (NO CACHE)", "thetadata_nocache.prof", clear_cache_first=True
    )

    # 2. ThetaData with cache warmed
    results["thetadata_cached"] = profile_backtest(
        ThetaDataBacktesting, "ThetaData (CACHED)", "thetadata_cached.prof", clear_cache_first=False
    )

    # 3. Polygon with cache cleared
    results["polygon_nocache"] = profile_backtest(
        PolygonDataBacktesting, "Polygon (NO CACHE)", "polygon_nocache.prof", clear_cache_first=True
    )

    # 4. Polygon with cache warmed
    results["polygon_cached"] = profile_backtest(
        PolygonDataBacktesting, "Polygon (CACHED)", "polygon_cached.prof", clear_cache_first=False
    )
    ...
    theta_cache_benefit = (
        results["thetadata_nocache"] / results["thetadata_cached"] if results["thetadata_cached"] > 0 else 0
    )
    polygon_cache_benefit = (
        results["polygon_nocache"] / results["polygon_cached"] if results["polygon_cached"] > 0 else 0
    )
