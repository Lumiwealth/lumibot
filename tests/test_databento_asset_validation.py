...
class TestDataBentoAssetValidation:
    ...
    def test_helper_function_allows_all_assets(self):
        ...
        for asset in test_assets:
            with patch("lumibot.tools.databento_helper.DATABENTO_AVAILABLE", True), patch(
                "lumibot.tools.databento_helper.DataBentoClient"
            ) as mock_client:
                ...
                try:
                    get_price_data_from_databento(
                        api_key="test_key", asset=asset, start=start_date, end=end_date, timestep="minute"
                    )
                    ...
                    if "only supports futures assets" in str(e):
                        pytest.fail(
                            f"Helper function should not validate asset types, but rejected {asset.asset_type}: {e}"
                        )
                    else:
